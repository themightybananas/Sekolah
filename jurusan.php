<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8" />
        <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1" />
        <link rel="icon" href="./assets/img/smki.png">
        <title>SMKI</title>
        <meta content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0, shrink-to-fit=no"name="viewport"/>
          <link rel="stylesheet" href="./assets/css/container.css">
        <link rel="stylesheet" href="./assets/css/jurusan.css" />
        <link rel="stylesheet" href="./assets/css/card.css" />
        <link href="./assets/css/bootstrap.min.css" rel="stylesheet" />
        <script src="./assets/js/slider.js"></script>
        <script src="./node_modules/bootstrap/dist/js/bootstrap.bundle.min.js"></script>
      </head>
      <body>

    <!-- NAVBAR -->
    <nav class="navbar navbar-expand-lg navbar-light bg-light py-4 px-3">
      <div class="container-fluid px-0">
        <a class="navbar-brand" href="../index.php">
          <img src="./assets/img/smki.png" alt="SMKI Logo" />
        </a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbar-default" aria-controls="navbar-default" aria-expanded="false" aria-label="Toggle navigation">
          <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbar-default">
          <ul class="navbar-nav me-auto mb-2 mb-lg-0">
            <li class="nav-item">
              <a class="nav-link" aria-current="page" href="index.php">Beranda</a>
            </li>
            <li class="nav-item dropdown">
              <a class="nav-link active dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">Informasi Jurusan</a>
              <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                <li><a class="dropdown-item" href="jurusan.php">Akutansi & Keuangan Lembaga</a></li>
                <li><a class="dropdown-item" href="jurusan.php">Pemasaran</a></li>
                <li><a class="dropdown-item" href="jurusan.php">Perbankan Syari'ah</a></li>
                <li><a class="dropdown-item" href="jurusan.php">Teknik Komputer & Jaringan</a></li>
                <li><a class="dropdown-item" href="jurusan.php">Pengembangan Perangkat Lunak & Gim</a></li>
                <li><a class="dropdown-item" href="jurusan.php">Desain Komunikasi Visual</a></li>
              </ul>
            </li>
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown2" role="button" data-bs-toggle="dropdown" aria-expanded="false">Berita</a>
              <ul class="dropdown-menu" aria-labelledby="navbarDropdown2">
                <li><a class="dropdown-item" href="berita.php">Prestasi</a></li>
                <li><a class="dropdown-item" href="berita.php">Kegiatan Sekolah</a></li>
                <li><a class="dropdown-item" href="berita.php">Kegiatan Organisasi</a></li>
                <li><a class="dropdown-item" href="berita.php">Ekstrakulikuler</a></li>
              </ul>
            </li>
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown3" role="button" data-bs-toggle="dropdown" aria-expanded="false">Fasilitas</a>
              <ul class="dropdown-menu" aria-labelledby="navbarDropdown3">
                <li><a class="dropdown-item" href="sarana.php">Sarana</a></li>
                <li><a class="dropdown-item" href="fasilitas.php">Prasarana</a></li>
              </ul>
            </li>
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown4" role="button" data-bs-toggle="dropdown" aria-expanded="false">Layanan</a>
              <ul class="dropdown-menu" aria-labelledby="navbarDropdown4">
                <li><a class="dropdown-item" href="#">SIAKAD</a></li>
                <li><a class="dropdown-item" href="#">Penelusuran Alumni</a></li>
                <li><a class="dropdown-item" href="#">TEFA Kami</a></li>
                <li><a class="dropdown-item" href="#">Ruang Praktek Siswa</a></li>
                <li><a class="dropdown-item" href="#">Kerjasama DUDI</a></li>
              </ul>
            </li>
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown5" role="button" data-bs-toggle="dropdown" aria-expanded="false">Pendaftaran Siswa Baru</a>
              <ul class="dropdown-menu" aria-labelledby="navbarDropdown5">
                <li><a class="dropdown-item" href="#">Daftar</a></li>
                <li><a class="dropdown-item" href="#">Cek Nama Kamu</a></li>
              </ul>
            </li>
          </ul>
          <div class="d-flex justify-content-end justify-content-lg-end w-100">
            <div class="d-flex align-items-center flex-column flex-lg-row">
              <a href="https://smkislambatu.sch.id/pages/sign-in.php" class="btn btn-primary ml-2 d-block">Masuk</a>
            </div>
          </div>
        </div>
      </div>
    </nav>


    




  

<!-- HEADER -->
<div class="container-fluid py-5 d-flex justify-content-center bg-custom ">
  <div class="p-5 mb-4 lc-block col-xxl-7 col-lg-8 col-12 custom-backdrop">
    <div class="lc-block d-flex align-items-center">
      <img src="./assets/img/smki.png" alt="Logo" class="logo-icon mr-3" style="width: 70px; height: auto;">
      <div editable="rich">
        <h2 class="font-weight-bold display-3">JURUSAN</h2>
      </div>
    </div>
    <div class="lc-block col-md-12">
      <div editable="rich">
        <p class="lead">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Corporis dolore nam unde distinctio nobis repellat fuga quis sapiente repellendus explicabo, porro voluptatibus nulla provident! Qui voluptatum temporibus amet et provident?</p>
      </div>
    </div>
  </div>
</div>













<!-- INFO -->
<div class="bg-dark py-5">
  <div class="container bg-dark">
    <div class="row align-items-center">
      <div class="col-xl-6 mb-4 mb-xl-0">
        <h2 class="font-weight-bold display-4 text-warning">Informasi Jurusan</h2>
        <p class="text-white text-justify">
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc et metus id ligula malesuada. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc et metus id ligula malesuadaLorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc et metus id ligula malesuadaplacerat sit amet quis enim. Aliquam erat volutpat. In pellentesque scelerisque auctor. Ut porta lacus eget nisi fermentum lobortis.
        </p>
      </div>
      <div class="col-xl-6">
        <div class="row g-2">
          <div class="col-6 col-md-6 mb-4">
            <div class="card card-body shadow border-0 d-flex flex-column align-items-center" style="min-height:100px;">
              <div class="mb-3 pt-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M4,2A1,1 0 0,0 3,3V4A1,1 0 0,0 4,5H5V14H11V16.59L6.79,20.79L8.21,22.21L11,19.41V22H13V19.41L15.79,22.21L17.21,20.79L13,16.59V14H19V5H20A1,1 0 0,0 21,4V3A1,1 0 0,0 20,2H4Z"/>
                </svg>
              </div>
              <div class="text-center">
                <span id="counter-project" class="fw-bold h4 mb-4">125</span>
                <p class="lead h3"><strong>Proyek</strong></p>
              </div>
            </div>
          </div>
          <div class="col-6 col-md-6 mb-4">
            <div class="card card-body shadow border-0 d-flex flex-column align-items-center" style="min-height:100px;">
              <div class="mb-3 pt-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M16 17V19H2V17S2 13 9 13 16 17 16 17M12.5 7.5A3.5 3.5 0 1 0 9 11A3.5 3.5 0 0 0 12.5 7.5M15.94 13A5.32 5.32 0 0 1 18 17V19H22V17S22 13.37 15.94 13M15 4A3.39 3.39 0 0 0 13.07 4.59A5 5 0 0 1 13.07 10.41A3.39 3.39 0 0 0 15 11A3.5 3.5 0 0 0 15 4Z"/>
                </svg>
              </div>
              <div class="text-center">
                <span id="counter-student" class="fw-bold h4 mb-4">53</span>
                <p class="lead h3"><strong>Siswa</strong></p>
              </div>
            </div>
          </div>
          <div class="col-6 col-md-6 mb-4">
            <div class="card card-body shadow border-0 d-flex flex-column align-items-center" style="min-height:100px;">
              <div class="mb-3 pt-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12,10A3.5,3.5 0 0,0 15.5,6.5A3.5,3.5 0 0,0 12,3A3.5,3.5 0 0,0 8.5,6.5A3.5,3.5 0 0,0 12,10M12,5A1.5,1.5 0 0,1 13.5,6.5A1.5,1.5 0 0,1 12,8A1.5,1.5 0 0,1 10.5,6.5A1.5,1.5 0 0,1 12,5M15,20A1,1 0 0,0 14,19H13V17H19V15.5C19,13.57 15.87,12 12,12C8.13,12 5,13.57 5,15.5V17H11V19H10A1,1 0 0,0 9,20H2V22H9A1,1 0 0,0 10,23H14A1,1 0 0,0 15,22H22V20H15M7.61,15C8.39,14.53 9.89,14 12,14C14.11,14 15.61,14.53 16.39,15H7.61Z"/>
                </svg>
              </div>
              <div class="text-center">
                <span id="counter-employee" class="fw-bold h4 mb-4">250</span>
                <p class="lead h3"><strong>Karyawan</strong></p>
              </div>
            </div>
          </div>
          <div class="col-6 col-md-6 mb-4">
            <div class="card card-body shadow border-0 d-flex flex-column align-items-center" style="min-height:100px;">
              <div class="mb-3 mt-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="3em" height="3em" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M5,3V21H11V17.5H13V21H19V3H5M7,5H9V7H7V5M11,5H13V7H11V5M15,5H17V7H15V5M7,9H9V11H7V9M11,9H13V11H11V9M15,9H17V11H15V9M7,13H9V15H7V13M11,13H13V15H11V13M15,13H17V15H15V13M7,17H9V19H7V17M15,17H17V19H15V17Z"/>
                </svg>
              </div>
              <div class="text-center">
                <span id="counter-lab" class="fw-bold h4 mb-4">5000</span>
                <p class="lead h3"><strong>Lab</strong></p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>


















<!--MORE INFO-->
<div class="bg-dark shadow">
  <div class="row row-cols-1 row-cols-lg-2 g-0 overflow-hidden p-5 text-white">
    <div class="col-lg-6 col-12">
      <div class="lc-block p-4">
        <div class="d-flex align-items-center mb-4">
          <svg xmlns="http://www.w3.org/2000/svg" class="mr-3" width="3em" height="3em" fill="currentColor" viewBox="0 0 16 16" style="max-width: 100%; height: auto;">
            <path d="M8.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L2.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093L8.95 4.992zm-.92 5.14.92.92a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 1 0-1.091-1.028L9.477 9.417l-.485-.486z"/>
          </svg>
          <h1 class="fw-bold text-warning mb-0">Peluang Kerja</h1>
        </div>
        <p class="fw-medium">Lorem ipsum dolor sit amet consectetur adipisicing elit. Aut eligendi ut incidunt
           commodi blanditiis doloribus at fugiat sequi velit maiores quod tempore, suscipit nemo totam, eveniet cum, esse consectetur repellendus.</p>
        <ul class="list-unstyled custom-list">
          <li class="mb-2 d-flex align-items-center">
            <i class="bi bi-check-circle me-2 text-white"></i> 
            Mengembangkan kurikulum yang sesuai dengan kebutuhan siswa
          </li>
          <li class="mb-2 d-flex align-items-center">
            <i class="bi bi-check-circle me-2 text-white"></i> 
            Meningkatkan kemampuan guru dalam mengajar
          </li>
          <li class="mb-2 d-flex align-items-center">
            <i class="bi bi-check-circle me-2 text-white"></i> 
            Mengembangkan fasilitas sekolah yang memadai
          </li>
        </ul>
      </div>
    </div>
    <div class="col-lg-6 col-12">
      <div class="lc-block p-4">
        <div class="d-flex align-items-center mb-4">
          <svg xmlns="http://www.w3.org/2000/svg" class="mr-3" width="3em" height="3em" fill="currentColor" viewBox="0 0 16 16" style="max-width: 100%; height: auto;">
            <path d="M14 9.5a2 2 0 1 1-4 0 2 2 0 0 1 4 0m-6 5.7c0 .8.8.8.8.8h6.4s.8 0 .8-.8-.8-3.2-4-3.2-4 2.4-4 3.2"/>
            <path d="M2 2a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h5.243c.122-.326.295-.668.526-1H2a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v7.81c.353.23.656.496.91.783Q16 12.312 16 12V4a2 2 0 0 0-2-2z"/>
          </svg>
          <h1 class="fw-bold text-warning mb-0">Pembelajaran</h1>
        </div>
        <p class="fw-medium">Lorem ipsum dolor sit amet consectetur adipisicing elit. Aut eligendi ut incidunt commodi blanditiis doloribus at fugiat sequi velit maiores quod tempore, suscipit nemo totam, eveniet cum, esse consectetur repellendus.</p>
        <ul class="list-unstyled custom-list">
          <li class="mb-2 d-flex align-items-center">
            <i class="bi bi-check-circle me-2 text-white"></i> 
            Mengembangkan kurikulum yang sesuai dengan kebutuhan siswa
          </li>
          <li class="mb-2 d-flex align-items-center">
            <i class="bi bi-check-circle me-2 text-white"></i> 
            Meningkatkan kemampuan guru dalam mengajar
          </li>
          <li class="mb-2 d-flex align-items-center">
            <i class="bi bi-check-circle me-2 text-white"></i> 
            Mengembangkan fasilitas sekolah yang memadai
          </li>
        </ul>
      </div>
    </div>
  </div>
</div>






















<!--PROFILE-->
<div class="container-fluid bg-dark p-5">
  <div class="container mb-5">
    <div class="row g-0">
      <div class="col-lg-6">
        <div class="lc-block">
          <img src="./assets/img/bg3.jpg" class="img-fluid" style="border-radius: 20px;" />
        </div>
      </div>
      <div class="col-lg-6 d-flex align-items-center bg-light" style="border-radius: 20px;">
        <div class="lc-block p-2" style="margin: 45px;">
          <h2 class="fw-bold d-inline-block">NAMA KAJUR</h2>
          <span class="ms-2 text-uppercase font-weight-bold text-nowrap">AKL</span>
          <p class="mt-3 text-justify">
            Using a series of utilities, you can create this jumbotron, just like the one in previous versions of Bootstrap.
          </p>
          <ul class="social-list list-inline mb-0 mt-4">
            <li class="list-inline-item">
              <a class="text-dark" href="#">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="blue" class="bi bi-facebook" viewBox="0 0 16 16">
                  <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951"/>
                </svg>
              </a>
            </li>
            <li class="list-inline-item">
              <a class="text-dark" href="#">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="red" class="bi bi-instagram" viewBox="0 0 16 16">
                  <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.9 3.9 0 0 0-1.417.923A3.9 3.9 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.9 3.9 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.9 3.9 0 0 0-.923-1.417A3.9 3.9 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599s.453.546.598.92c.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.5 2.5 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.5 2.5 0 0 1-.92-.598 2.5 2.5 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233s.008-2.388.046-3.231c.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92s.546-.453.92-.598c.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92m-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217m0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334"/>
                </svg>
              </a>
            </li>
          </ul>
        </div>
      </div>
    </div>
  </div>  
  <div class="container mb-4">
    <div class="d-flex align-items-center">
      <h3 class="mb-2 text-white">Karyawan Jurusan</h3>
      <div class="flex-grow-1">
        <hr class="ml-2 m-0" style="border-color: yellow;">
      </div>
    </div>
  </div>  
  <div class="row g-3">
    <div class="col-lg-6 col-md-12 col-12 mb-4 mt-4">
      <div class="card d-flex flex-md-row flex-column" style="border-radius: 20px; border: none;">
        <div class="d-flex">
          <img src="./assets/img/bg3.jpg" class="img-fluid" alt="..." style="border-radius: 20px 0 0 20px; border: none;">
        </div>
        <div class="card-body p-4">
          <h4 class="border-bottom border-info mb-3">oke</h4>
          <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
          <a href="guru.php" class="btn btn-link" role="button">Lebih Lengkap</a>
        </div>
      </div>
    </div>
    <div class="col-lg-6 col-md-12 col-12 mb-4 mt-4">
      <div class="card d-flex flex-md-row flex-column" style="border-radius: 20px; border: none;">
        <div class="d-flex">
          <img src="./assets/img/bg3.jpg" class="img-fluid" alt="..." style="border-radius: 20px 0 0 20px; border: none;">
        </div>
        <div class="card-body p-4">
          <h4 class="border-bottom border-info mb-3">oke</h4>
          <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
          <a href="guru.php" class="btn btn-link" role="button">Lebih Lengkap</a>
        </div>
      </div>
    </div>
    <div class="col-lg-6 col-md-12 col-12 mb-4 mt-4">
      <div class="card d-flex flex-md-row flex-column" style="border-radius: 20px; border: none;">
        <div class="d-flex">
          <img src="./assets/img/bg3.jpg" class="img-fluid" alt="..." style="border-radius: 20px 0 0 20px; border: none;">
        </div>
        <div class="card-body p-4">
          <h4 class="border-bottom border-info mb-3">oke</h4>
          <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
          <a href="guru.php" class="btn btn-link" role="button">Lebih Lengkap</a>
        </div>
      </div>
    </div>
    <div class="col-lg-6 col-md-12 col-12 mb-4 mt-4">
      <div class="card d-flex flex-md-row flex-column karyawan-card" style="border-radius: 20px; border: none;">
        <div class="d-flex">
          <img src="./assets/img/bg3.jpg" class="img-fluid" alt="..." style="border-radius: 20px 0 0 20px; border: none;">
        </div>
        <div class="card-body p-4">
          <h4 class="border-bottom border-info mb-3">oke</h4>
          <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
          <a href="guru.php" class="btn btn-link" role="button">Lebih Lengkap</a>
        </div>
      </div>
    </div>   
  </div>  
  
  
</div>
  






















<script>
  function initializeCounterRANDOMID(){
    
    const options = {};
    
    const observer = new IntersectionObserver(function
    (entries, observer){
        entries.forEach(entry => {
            console.log(entry);
             VanillaCounter();
        });
    }, options);
    
    observer.observe(document.querySelector('.counter-RANDOM'));
  }
</script>







      <!-- FOOTER -->
      <footer class="pt-lg-5 pt-3 footer bg-light">
        <div class="container">
          <div class="row">
            <!-- Company Info -->
            <div class="col-12 col-md-6 col-lg-3 mb-4">
              <div class="text-center">
                <img src="./assets/img/smki.png" alt="School Image" class="img-fluid mt-4" style="max-width: 170px;" />
              </div>
            </div>            
            <!-- Informasi Jurusan -->
            <div class="col-12 col-md-6 col-lg-4 mb-4">
              <div class="d-flex align-items-center mb-3">
                <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="30px" fill="blue" class="me-2">
                  <path d="M431.52-271.52h96.96V-520h-96.96v248.48ZM480-588.7q21.8 0 36.55-14.75Q531.3-618.2 531.3-640q0-21.8-14.75-36.55Q501.8-691.3 480-691.3q-21.8 0-36.55 14.75Q428.7-661.8 428.7-640q0 21.8 14.75 36.55Q458.2-588.7 480-588.7Zm0 527.92q-87.52 0-163.91-32.96-76.38-32.96-132.88-89.47-56.51-56.5-89.47-132.88Q60.78-392.48 60.78-480t32.96-163.91q32.96-76.38 89.47-132.88 56.5-56.51 132.88-89.47 76.39-32.96 163.91-32.96t163.91 32.96q76.38 32.96 132.88 89.47 56.51 56.5 89.47 132.88 32.96 76.39 32.96 163.91t-32.96 163.91q-32.96 76.38-89.47 132.88-56.5 56.51-132.88 89.47Q567.52-60.78 480-60.78Zm0-106q131.74 0 222.48-90.74 90.74-90.74 90.74-222.48t-90.74-222.48Q611.74-793.22 480-793.22t-222.48 90.74Q166.78-611.74 166.78-480t90.74 222.48q90.74 90.74 222.48 90.74ZM480-480Z"/>
                </svg>
                <h4 class="fw-bold text-dark mb-0 border-bottom">Informasi Jurusan</h4>
              </div>
              <ul class="list-unstyled">
                <li><a href="#" class="nav-link p-0 mb-2" style="color: black;">Akutansi & Keuangan Lembaga</a></li>
                <li><a href="#" class="nav-link p-0 mb-2" style="color: black;">Pemasaran</a></li>
                <li><a href="#" class="nav-link p-0 mb-2" style="color: black;">Perbankan Syar'ah</a></li>
                <li><a href="#" class="nav-link p-0 mb-2" style="color: black;">Teknik Komputer & Jaringan</a></li>
                <li><a href="#" class="nav-link p-0 mb-2" style="color: black;">Pengembangan Perangkat Lunak & Gim</a></li>
                <li><a href="#" class="nav-link p-0 mb-2" style="color: black;">Desain Komunikasi Visual</a></li>
              </ul>
            </div>
        
            <!-- Berita -->
            <div class="col-12 col-md-6 col-lg-2 mb-4">
              <div class="d-flex align-items-center mb-3">
                <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" class="me-2" width="30px" fill="red">
                  <path d="M45.52-147v-106h868.96v106H45.52Zm67-173v-346h106v346h-106Zm176.96 0v-506h106v506h-106ZM467-320v-506h106v506H467Zm313.91 0-173-303.18 93.18-53 173 303.18-93.18 53Z"/>
                </svg>
                <h4 class="fw-bold text-dark mb-0 border-bottom">Berita</h4>
              </div>
              <ul class="list-unstyled">
                <li><a href="#" class="nav-link p-0 mb-2" style="color: black;">Prestasi</a></li>
                <li><a href="#" class="nav-link p-0 mb-2" style="color: black;">Kegiatan Sekolah</a></li>
                <li><a href="#" class="nav-link p-0 mb-2" style="color: black;">Kegiatan Organisasi</a></li>
                <li><a href="#" class="nav-link p-0 mb-2" style="color: black;">Ekstrakulikuler</a></li>
              </ul>
            </div>
        
            <!-- Contact Info -->
            <div class="col-12 col-md-6 col-lg-3 mb-4">
              <div class="d-flex align-items-center mb-3">
                <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="green" class="bi bi-telephone-inbound-fill me-2" viewBox="0 0 16 16">
                  <path fill-rule="evenodd" d="M1.885.511a1.745 1.745 0 0 1 2.61.163L6.29 2.98c.329.423.445.974.315 1.494l-.547 2.19a.68.68 0 0 0 .178.643l2.457 2.457a.68.68 0 0 0 .644.178l2.189-.547a1.75 1.75 0 0 1 1.494.315l2.306 1.794c.829.645.905 1.87.163 2.611l-1.034 1.034c-.74.74-1.846 1.065-2.877.702a18.6 18.6 0 0 1-7.01-4.42 18.6 18.6 0 0 1-4.42-7.009c-.362-1.03-.037-2.137.703-2.877zM15.854.146a.5.5 0 0 1 0 .708L11.707 5H14.5a.5.5 0 0 1 0 1h-4a.5.5 0 0 1-.5-.5v-4a.5.5 0 0 1 1 0v2.793L15.146.146a.5.5 0 0 1 .708 0"/>
                </svg>
                <h4 class="fw-bold text-dark mb-0 border-bottom">Kontak Kami</h4>
              </div>
              <p class="mb-0">JL. Barat Stadion Brantas Batu</p>
              <p class="mb-0">Kode Pos : 65314</p>
              <p>Email : <a href="" class="text-dark">smkislambatu@gmail.com</a></p>
              <p>Phone: <span class="fw-semibold">(000) 123 456 789</span></p>
            </div>
          <div class="row align-items-center g-0 py-2 mt-4">
            <div class="rounded">
            <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css">
            <a href="https://api.whatsapp.com/send?phone=51955081075&text=Hola%21%20Quisiera%20m%C3%A1s%20informaci%C3%B3n%20sobre%20Varela%202." class="float" target="_blank">
            <i class="fa fa-whatsapp my-float"></i>
            </a>
          </div>
          </div>
        </div>
      </footer>  
    </body>
  </html>










